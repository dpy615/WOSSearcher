...
namespace WebSearcher {
    public class UniversityOfWarwick : Searcher {
        ...
        public override void GetRes(object o) {
            ...
            for (int i = start; i < end; i++) {
                ...
                try {
                    ...
                    string uri = "http://wrap.warwick.ac.uk/cgi/search/simple?q="+title+"&_action_search=Search&_order=bytitle&basic_srchtype=ALL&_satisfyall=ALL";
                    ...
                    //res = DealStr(res);
                    if (res.ToLower().Contains(("<em>"+title_old).ToLower())) {
                        dt.Rows[i]["isIn"] = "YES";
                    } else {
                        dt.Rows[i]["isIn"] = "NO";
                    }
