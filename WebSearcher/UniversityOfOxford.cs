...
namespace WebSearcher {
    public class UniversityOfOxford : Searcher {
        ...
        public override void GetRes(object o) {
            ...
            for (int i = start; i < end; i++) {
                ...
                try {
                    ...
                    string title = title_old.Replace(' ', '+').Replace("&", "%26").Replace(",", "%2C").Replace("'","%27").Replace("?","%3F");

                    string uri = "http://ora.ox.ac.uk/search/detailed?q="+title+"&filtercontainsFullText=&search=SEARCH";
        ...
        private string DealStr(string str_o) {
            string str = str_o;
            str = str.Replace("\r", "").Replace("\n", "").Replace("&#39;", "'").Replace("</B>", "").Replace("<EM>", "").Replace("</EM>", "");
