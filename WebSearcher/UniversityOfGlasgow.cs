...
namespace WebSearcher {
    public class UniversityOfGlasgow : Searcher {
        ...
        public override void GetRes(object o) {
            ...
            for (int i = start; i < end; i++) {
                ...
                try {
                    ...
                    string title = title_old.Replace(' ', '+').Replace('&',' ');

                    string uri = "http://boris.unibe.ch/cgi/search/simple?q="+title+"&_action_search=Search&_action_search=Search&_order=bytitle&basic_srchtype=ALL&_satisfyall=ALL&limit=5";
                    ...
                    //res = DealStr(res);
                    if (res.ToLower().Contains((title_old+".").ToLower())) {
                        dt.Rows[i]["isIn"] = "YES";
                    } else {
                        dt.Rows[i]["isIn"] = "NO";
                    }
